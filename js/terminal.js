...
export function initTerminal(){
  ...
  window.addEventListener('keydown', (e)=>{
    ...
    if(e.key === 'Tab'){
      ...
      // Fuzzy match helper - matches if target contains all chars of query in order
      ...
      if(parts.length === 1){
        ...
        let hit = CMDS.find(c => c.startsWith(base));
        if(!hit) hit = CMDS.find(c => fuzzyMatch(base, c));
        if(hit) termInput.value = hit + " ";
      } else if(parts.length === 2 && parts[0].toLowerCase() === 'open'){
        const base = parts[1].toLowerCase();
        const keys = Object.keys(APPS);
        let hit = keys.find(k => k.startsWith(base));
        if(!hit) hit = keys.find(k => fuzzyMatch(base, k));
...
function openApp(key, kind="open"){
  const a = APPS[key];
  if(!a){
    termPrint(`error: unknown app "${key}"`, "err");
    return false;
  }

  // Navigate to the app's HTML file
  termPrint(`launching ${a.name.toLowerCase()}...`, "m");
  setTimeout(() => {
    try {
      window.location.href = a.href;
    } catch(e) {
      termPrint(`error: could not load ${a.href}`, "err");
    }
  }, 120);
  return true;
}
